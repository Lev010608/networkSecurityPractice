...
/**
 * 模拟 Diffie-Hellman 密钥交换算法，包括密钥生成、密钥交换和攻击者模拟破解。
 */
public class DiffieHellmanSimulation {

    /**
     * 主函数，负责用户输入、密钥生成、密钥交换、攻击者模拟等过程的调度。
     *
     * @param args 程序参数（未使用）
     */
    ...
    private static BigInteger generateLargePrime(int bitLength) {
        ...
        // 使用 isProbablePrime 方法检查生成的数是否可能是素数
     ...
     *
     * @param publicKey    公开密钥 Y
     * @param privateKey   私钥 X
     * @param p            大素数 p
     * @return 计算得到的共享密钥 K
     */
     ...
     * @param p              大素数 p
     * @param a              大素数 p 的元根
     * @param targetPublicKeyA Alice公开密钥
     * @param targetPublicKeyB Bob公开密钥
     */
    private static void simulateAttacker(BigInteger p, BigInteger a, BigInteger targetPublicKeyA,
                                         BigInteger targetPublicKeyB) {
        Scanner scanner = new Scanner(System.in);
        System.out.println("模拟攻击者开始尝试破解 Alice 的私钥 X_A,输入回车开始破解：");
        // 模拟攻击者尝试破解 Alice 的私钥 X_A
        scanner.nextLine();
        attack(p, a, targetPublicKeyA);

        System.out.println("模拟攻击者开始尝试破解 Bob 的私钥 X_B，,输入回车开始破解：");
        scanner.nextLine();
        // 模拟攻击者尝试破解 Bob 的私钥 X_B
        attack(p, a, targetPublicKeyB);
    }
    ...
    private static void attack(BigInteger p, BigInteger a, BigInteger targetPublicKey) {
        ...
        while (true) {
            ...
            // 输出每次尝试的私钥值和对应的公开密钥
            System.out.println("尝试次数: " + attempts + ", 私钥值: " + x + ", 计算得到的公开密钥: " + computedPublicKey + ", 正确的公开密钥: "
                    + targetPublicKey);
            ...
            if (computedPublicKey.equals(targetPublicKey)) {
                ...
                System.out.println("成功！尝试次数: " + attempts + ", 破译所花时间: " + elapsedTime + " 毫秒, 计算得到的公开密钥: "
                        + computedPublicKey + ", 正确的公开密钥: " + targetPublicKey);
                break;
            }
